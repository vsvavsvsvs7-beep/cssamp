...
# Pastikan API Key ini yang terbaru dari Google AI Studio
...
class TatangBot(commands.Bot):
    ...
    async def on_ready(self):
        await self.change_presence(activity=discord.Game(name="!menu | Premium CS"))
...
class CSModal(discord.ui.Modal, title="Form Character Story"):
    ...
    detail = discord.ui.TextInput(
        ...
        placeholder="Tuliskan latar belakang karaktermu...",
        max_length=2000,
        required=True
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        # Wajib defer agar tidak 'Something went wrong'
        ...
        prompt = (
            ...
            "WAJIB: Minimal 500 kata, gunakan BBCode [center] [justify] [b]."
        )
        
        # Taktik Anti-404: Mencoba 2 format model berbeda
        hasil_cerita = None
        error_log = ""
        ...
        for model_name in ['gemini-1.5-flash', 'models/gemini-1.5-flash']:
            try:
                ...
                if hasil_cerita: break
            except Exception as e:
                error_log = str(e)
                continue

        if not hasil_cerita:
            return await interaction.followup.send(f"❌ Gagal memanggil AI: {error_log}", ephemeral=True)
        
        try:
            # Membuat file TXT
            isi_file = f"--- CS {self.server} ---\n\n{hasil_cerita}"
            ...
            await interaction.followup.send(f"❌ Error saat kirim file: {str(e)}", ephemeral=True)
...
class ServerSelect(discord.ui.Select):
    def __init__(self):
        options = [discord.SelectOption(label=s) for s in ["JGRP", "SSRP", "Virtual RP", "AARP", "GCRP"]]
